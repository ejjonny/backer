...
impl<'a, State> NodeValue<'a, State> {
...
impl SizeConstraints {
    pub(crate) fn from_size<State>(value: Size<State>, area: Area, a: &mut State) -> Self {
        ...
        if let Some(dynamic) = value.dynamic_height {
            let result = Some(initial.height.clamp(dynamic(area.width, a)));
            initial.height.lower = result;
            initial.height.upper = result;
        }
        if let Some(dynamic) = value.dynamic_width {
            let result = Some(initial.width.clamp(dynamic(area.height, a)));
