...
pub(crate) struct Subtree<SubState, State: Scopable<Scoped = SubState>> {
    pub(crate) subtree_fn: SubtreeFn<SubState>,
    pub(crate) stored_tree: Option<Node<SubState>>,
    pub(crate) _p: PhantomData<State>,
}

impl<SubState, State: Scopable<Scoped = SubState>> NodeTrait<State> for Subtree<SubState, State> {
