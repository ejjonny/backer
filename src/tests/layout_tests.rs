...
mod tests {
    ...
    fn test_partial_scope_variadic() {
        ...
        impl<'a, 'b> Scopable for TupleA<'a>
        where
            Self: 'a,
            Self::Scoped: 'b,
            'a: 'b,
        {
            type Scoped = Unscopable<TupleB<'b>>;
            fn with_scoped<T>(
                &mut self,
                subtree: &mut crate::subtree::Subtree<Self>,
                f: impl Fn(&mut crate::subtree::Subtree<Self>, &mut Self::Scoped) -> T,
            ) -> T {
                let substate = &mut (&mut *self.0, &mut self.1.c);
                f(subtree, substate)
            }
        }
        // impl<'a> Scopable for TupleB<'a> {
        //     type Scoped = ();
        //     fn with_scoped<T>(
        //         &mut self,
        //         subtree: &mut crate::subtree::Subtree<Self>,
        ...
        .draw(Area::new(0., 0., 100., 100.), &mut tuple);
    }
}
